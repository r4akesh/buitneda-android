...
class MyReviewListActivity : BaseActivity() {
    ...
    private fun callApi() {
        ...
        mHashIdentifier = Utils.getMd5String("getReviewList" + AppSharedPref.getStoreId(this) + AppSharedPref.getCustomerToken(this) + mPageNumber)
        ApiConnection.getReviewsList(this, mDataBaseHandler.getETagFromDatabase(mHashIdentifier), mPageNumber++, false)
    ...
    private fun checkAndLoadLocalData() {
        ...
        if (response.isNotBlank()) {
            val reviewListResponseModel: ReviewListResponseModel = mGson.fromJson(response, ReviewListResponseModel::class.java)
    ...
    private fun onSuccessfulResponse(reviewListResponseModel: ReviewListResponseModel) {
        if (mPageNumber == 2) {
            ...
            mContentViewBinding.reviewsListRv.adapter?.notifyItemRangeChanged(mContentViewBinding.data!!.reviewList.size - reviewListResponseModel.reviewList.size, reviewListResponseModel.reviewList.size)
        }
    }
    ...
    private fun setupReviewsListRv() {
        mContentViewBinding.reviewsListRv.adapter = MyReviewsListRvAdapter(this, mContentViewBinding.data!!.reviewList)
        mContentViewBinding.reviewsListRv.isNestedScrollingEnabled = false

        mContentViewBinding.reviewsListRv.addOnScrollListener(object : RecyclerView.OnScrollListener() {
