...
open class ProductDetailsActivity : BaseActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        mContentViewBinding = DataBindingUtil.setContentView(this, R.layout.activity_product_details)
        startInitialization()
    }
...
//            }
        ...
        supportFragmentManager.beginTransaction().replace(R.id.main_frame, fragment!!, fragment.javaClass.simpleName)
                .commit()
    }
    ...
    private fun setTemporaryData() {
        ...
        mContentViewBinding.productSliderViewPager.adapter = ProductPageSliderAdapter(this, mProductName, productImage)
        mContentViewBinding.productSliderDotsTabLayout.setupWithViewPager(mContentViewBinding.productSliderViewPager, true)
    ...
    open fun callApi() {
        ...
        ApiConnection.getProductPageData(this, mDataBaseHandler.getETagFromDatabase(mHashIdentifier), mProductId)
                ...
                .subscribe(object : ApiCustomCallback<ProductDetailsPageModel>(this, false) {
                    override fun onNext(productDetailsPageModel: ProductDetailsPageModel) {
                        ...
                        if (productDetailsPageModel.success) {
                            if (AppSharedPref.getRecentlyViewedProductsEnabled(this@ProductDetailsActivity) && NetworkHelper.isNetworkAvailable(this@ProductDetailsActivity)) {
                                mDataBaseHandler.addRecentlyViewed(AppSharedPref.getStoreId(this@ProductDetailsActivity), AppSharedPref.getCurrencyCode(this@ProductDetailsActivity), productDetailsPageModel.id, Gson().toJson(productDetailsPageModel))
                            }
                ...
                .subscribe(object : Observer<String> {
                    override fun onNext(response: String) {
                        if (response.isNotBlank()) {
                            onSuccessfulResponse(mGson.fromJson(response, ProductDetailsPageModel::class.java))
                        }
                    }
    ...
    private fun setupCartUpdate() {
        if (intent.hasExtra(BundleKeysHelper.BUNDLE_KEY_CART_DATA)) {
            val cartData: CartItem = intent.getParcelableExtra(BundleKeysHelper.BUNDLE_KEY_CART_DATA)!!
            mContentViewBinding.data?.qty = cartData.qty ?: "1"
        }
    }

    private fun setupImageGallery() {
        mContentViewBinding.productSliderViewPager.adapter = ProductPageSliderAdapter(this, mProductName, mContentViewBinding.data!!.imageGallery)
        mContentViewBinding.productSliderDotsTabLayout.setupWithViewPager(mContentViewBinding.productSliderViewPager, true)
    }

    private fun setupTierPriceGallery() {
        mContentViewBinding.tierPriceRv.adapter = TierPriceRvAdapter(this, mContentViewBinding.data!!.tierPrices)
    }
    ...
    private fun setupProductOptions() {
        ...
        if (mProductDetailsPageModel.isAuction) {
            ...
            mContentViewBinding.auctionDetailsContainer.setVisibility(View.VISIBLE)
            mContentViewBinding.productCustomOptionsContainer.setVisibility(View.VISIBLE)
            mContentViewBinding.otherProductOptionsContainer.setVisibility(View.VISIBLE)
            mContentViewBinding.floatingBuyLayout.setVisibility(View.VISIBLE)
...
//        var finalBidAmount = re.replace(bidAmount, "")
        ...
        if (mProductDetailsPageModel.remainingTime > 0 || mProductDetailsPageModel.auctionFinish || mProductDetailsPageModel.won) {
            val auctionDetailsLayoutBinding: AuctionDetailsLayoutBinding = AuctionDetailsLayoutBinding.inflate(layoutInflater, mContentViewBinding.auctionDetailsContainer, true)
            auctionDetailsLayoutBinding.showCurrentAuctionPrice = mProductDetailsPageModel.showCurrentAuctionPrice
            auctionDetailsLayoutBinding.nextMinEditTextAmount = mProductDetailsPageModel.auctionPriceData?.unformattedNextMinAmount//finalBidAmount
...
//                    mContentViewBinding.staticAddToCartBtnLayout.setVisibility(View.GONE)
                    mContentViewBinding.floatingBuyNowBtn.text = java.lang.String.format(Locale.US, getString(R.string.buy_with_x), mProductDetailsPageModel.winning?.amount)
                    mContentViewBinding.staticBuyNowBtn.text = java.lang.String.format(Locale.US, getString(R.string.buy_with_x), mProductDetailsPageModel.winning?.amount)
                } else {
                    auctionDetailsLayoutBinding.auctionFinish = true
                }
                auctionDetailsLayoutBinding.isWon = mProductDetailsPageModel.won
                auctionDetailsLayoutBinding.showAuctionDetails = mProductDetailsPageModel.winning?.isShowAuctionDetails
                ...
                auctionDetailsLayoutBinding.bidAmount = mProductDetailsPageModel.winning?.currentBidAmount
                ...
                auctionDetailsLayoutBinding.openAmount = mProductDetailsPageModel.winning?.openAmount
...
*/
                ...
                auctionDetailsLayoutBinding.winningMessage = mProductDetailsPageModel.auctionFinishLabel
                auctionDetailsLayoutBinding.auctionFinish = true
                ...
                auctionDetailsLayoutBinding.isWon = mProductDetailsPageModel.won
                auctionDetailsLayoutBinding.showAuctionDetails = mProductDetailsPageModel.showAuctionDetails
                auctionDetailsLayoutBinding.setAuctionTitle(mProductDetailsPageModel.auctionTitle)
                auctionDetailsLayoutBinding.bidAmount = mProductDetailsPageModel.auctionPriceData?.currentAcutionAmount
                auctionDetailsLayoutBinding.bidCount = mProductDetailsPageModel.auctionDetails?.bidCount
                ...
                auctionDetailsLayoutBinding.openAmount = mProductDetailsPageModel.auctionDetails?.openAmount
                auctionDetailsLayoutBinding.auctionDetails = mProductDetailsPageModel.auctionDetails
                auctionDetailsLayoutBinding.auctionPriceData = mProductDetailsPageModel.auctionPriceData
                auctionDetailsLayoutBinding.bidsTv.setOnClickListener(View.OnClickListener { mContentViewBinding.handler!!.onClickShowBidList("normal") })
                ...
                auctionDetailsLayoutBinding.bidNowBtn.setOnClickListener(View.OnClickListener {
                    ...
                    if (mProductDetailsPageModel.auctionPriceData?.bidAmount != null) {
                        if (AppSharedPref.isLoggedIn(this)) {
                            ...
                            mContentViewBinding.loading = true
                            ApiConnection.addBid(AppSharedPref.getStoreId(this), AppSharedPref.getCustomerToken(this), mProductDetailsPageModel.auctionform?.proName, mProductDetailsPageModel.auctionform?.entityId, mProductDetailsPageModel.auctionform?.productId, mProductDetailsPageModel.auctionPriceData?.unformattedNextMinAmount, if (mProductDetailsPageModel.auctionPriceData!!.isPlaceAsAuto) 1 else 0, mProductDetailsPageModel.auctionform?.autoBidAllowed, mProductDetailsPageModel.auctionform?.stopAuctionTimeStamp)
                                    .observeOn(AndroidSchedulers.mainThread())
                                    .subscribeOn(Schedulers.io())
                                    .subscribe(object : ApiCustomCallback<BaseModel>(this, false) {
                                        override fun onNext(orderListResponseModel: BaseModel) {
                                            ...
                                            if (orderListResponseModel != null && orderListResponseModel.success) {
                                                callApi()
                                                ToastHelper.showToast(this@ProductDetailsActivity, orderListResponseModel.message)
                                            } else {
                                                ToastHelper.showToast(this@ProductDetailsActivity, orderListResponseModel.message)
                                        ...
                                        override fun onError(e: Throwable) {
                                            super.onError(e)
                                            NetworkHelper.getErrorMessage(this@ProductDetailsActivity, e)
                                            mContentViewBinding.loading = false
//                                onErrorResponse(e)
                                        }
                                    })
                            ...
                            AlertDialogHelper.showNewCustomDialog(
                                    ...
                                    DialogInterface.OnClickListener { dialogInterface: DialogInterface, _: Int ->
                                        ...
                                        startActivity((application as MobikulApplication).getLoginAndSignUpActivity(this))
                                    },
...
//            }
    ...
    private fun startTimer(finishMinutes: Long, auctionDetailsLayoutBinding: AuctionDetailsLayoutBinding) {
        if (mAuctionCountDownTimer != null) {
            mAuctionCountDownTimer?.cancel()
        }
...
//                cancel()
    ...
    private fun setCustomOptionName(customOptIndex: Int) {
        try {
            /*String*/
            val newCustomOptionLabelSpannable = SpannableString(mContentViewBinding.data!!.customOptions[customOptIndex].title)
            newCustomOptionLabelSpannable.setSpan(ForegroundColorSpan(ContextCompat.getColor(this, R.color.text_color_secondary)), 0, newCustomOptionLabelSpannable.length, Spannable.SPAN_INCLUSIVE_INCLUSIVE)
            val customOptionsName = TextView(this)
            customOptionsName.typeface = TypefaceUtils.load(assets, ApplicationConstants.CALLIGRAPHY_FONT_PATH_SEMI_BOLD)
            ...
            if (mContentViewBinding.data!!.customOptions[customOptIndex].unformatted_default_price != 0.0) {
                var formattedDefaultPrice = mContentViewBinding.data!!.customOptions[customOptIndex].formatted_default_price
                if (mContentViewBinding.data!!.customOptions[customOptIndex].price_type != "fixed") {
                    var price = mContentViewBinding.data!!.finalPrice
                    val unformattedDefaultPrice = mContentViewBinding.data!!.customOptions[customOptIndex].unformatted_default_price!!
            ...
            customOptionNameSpannable.setSpan(ForegroundColorSpan(ContextCompat.getColor(this, R.color.colorAccent)), 0, customOptionNameSpannable.length, Spannable.SPAN_INCLUSIVE_INCLUSIVE)
            ...
            if (mContentViewBinding.data!!.customOptions[customOptIndex].is_require == 1) {
                ...
                requiredSign.setSpan(ForegroundColorSpan(Color.RED), 0, requiredSign.length, Spannable.SPAN_INCLUSIVE_INCLUSIVE)
    ...
    private fun loadCustomOptionFieldAreaType(customOptIndex: Int) {
        try {
            ...
            editTextView.background = ContextCompat.getDrawable(this, R.drawable.shape_rect_white_bg_black_border_1_dp)
            ...
            if (mContentViewBinding.data!!.customOptions[customOptIndex].max_characters != "0") {
                ...
                maxNoOfCharacters.setTextColor(ContextCompat.getColor(this, R.color.text_color_primary))
    ...
    private fun loadCustomOptionARFieldType(customOptIndex: Int) {
        try {
            val eachARFieldTypeCustomOptionView = layoutInflater.inflate(R.layout.custom_option_ar_field_view, mContentViewBinding.productCustomOptionsContainer, false)
            eachARFieldTypeCustomOptionView.tag = customOptIndex

            val arFieldEt = eachARFieldTypeCustomOptionView.findViewById<AppCompatEditText>(R.id.ar_field_et)
            ...
            val arButtonEt = eachARFieldTypeCustomOptionView.findViewById<AppCompatButton>(R.id.ar_button)
            if (mContentViewBinding.data!!.customOptions[customOptIndex].isAr && AppSharedPref.getIsArSupported(this)) {
                arButtonEt.setOnClickListener {
                    ...
                    if (ContextCompat.checkSelfPermission(this, Manifest.permission.CAMERA) == PackageManager.PERMISSION_GRANTED &&
                        ...
                        startActivityForResult(Intent(this, MeasurementActivity::class.java), RC_AR_MEASURE)
                    } else {
            ...
            mContentViewBinding.productCustomOptionsContainer.addView(eachARFieldTypeCustomOptionView)
    ...
    private fun loadCustomOptionFileType(customOptIndex: Int) {
        try {
            val eachFileTypeCustomOptionView = layoutInflater.inflate(R.layout.custom_option_file_view, mContentViewBinding.productCustomOptionsContainer, false)
            eachFileTypeCustomOptionView.tag = customOptIndex

            val fileNameTv = eachFileTypeCustomOptionView.findViewById<AppCompatTextView>(R.id.fileSelectedTV)
            fileNameTv.tag = "fileName$customOptIndex"
            fileNameTv.setTextColor(ContextCompat.getColor(this, R.color.text_color_secondary))

            val browseButton = eachFileTypeCustomOptionView.findViewById<AppCompatButton>(R.id.browseButton)
            browseButton.tag = customOptIndex
            browseButton.setOnClickListener {
                mSeletedCustomOption = customOptIndex
                if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.JELLY_BEAN_MR1 && ContextCompat.checkSelfPermission(this, android.Manifest.permission.READ_EXTERNAL_STORAGE) != PackageManager.PERMISSION_GRANTED) {
                    ActivityCompat.requestPermissions(this, arrayOf(android.Manifest.permission.READ_EXTERNAL_STORAGE), RC_WRITE_TO_EXTERNAL_STORAGE)
                } else {
                    showFileChooser()
                }
            }
            ...
            val fileExtension = " <b>" + mContentViewBinding.data!!.customOptions[customOptIndex].file_extension + "</b>"
            ...
            if (mContentViewBinding.data!!.customOptions[customOptIndex].file_extension != "null") {
                ...
                allowedFileExt.typeface = TypefaceUtils.load(assets, ApplicationConstants.CALLIGRAPHY_FONT_PATH_REGULAR)
                allowedFileExt.setTextColor(ContextCompat.getColor(this, R.color.text_color_secondary))
                allowedFileExt.text = Html.fromHtml(resources.getString(R.string.allowed_file_extensions_to_upload_) + fileExtension)
            ...
            if (mContentViewBinding.data!!.customOptions[customOptIndex].image_size_x != "0") {
                ...
                imageSizeX.typeface = TypefaceUtils.load(assets, ApplicationConstants.CALLIGRAPHY_FONT_PATH_REGULAR)
                ...
                val imageSizeXValue = "<b>" + mContentViewBinding.data!!.customOptions[customOptIndex].image_size_x + "</b>"
                imageSizeX.text = Html.fromHtml(resources.getString(R.string.maximum_image_width) + imageSizeXValue)
    ...
    private fun showFileChooser() {
        ...
        intent.type = "image/*"
        try {
            startActivityForResult(Intent.createChooser(intent, getString(R.string.select_picture)), RC_PICK_SINGLE_FILE)
    ...
    private fun loadCustomOptionDropDownType(customOptIndex: Int) {
        try {
            ...
            val spinnerArrayAdapter = ArrayAdapter(this, android.R.layout.simple_spinner_dropdown_item, spinnerOptions)
    ...
    private fun loadCustomOptionRadioType(customOptIndex: Int) {
        try {
            ...
            if (mContentViewBinding.data!!.customOptions[customOptIndex].is_require == 0) {
                ...
                eachOptionInRadioTypeCustomOption.setTextColor(ContextCompat.getColor(this, R.color.text_color_primary))
            ...
            for (noOfOptions in optionValues!!.indices) {
                ...
                if (optionValues[noOfOptions].defaultPrice != 0.0) {
                    if (optionValues[noOfOptions].defaultPriceType == "fixed") {
                        optionString = SpannableString(" (+" + optionValues[noOfOptions].formattedDefaultPrice + ")")
                    } else {
                        var price = mContentViewBinding.data!!.finalPrice
                        val pattern = mContentViewBinding.data!!.priceFormat.pattern
                        val precision = mContentViewBinding.data!!.priceFormat.precision
                ...
                optionString.setSpan(ForegroundColorSpan(ContextCompat.getColor(this, R.color.colorAccent)), 0, optionString.length, Spannable.SPAN_INCLUSIVE_INCLUSIVE)
    ...
    private fun loadCustomOptionCheckBoxType(customOptIndex: Int) {
        try {
            ...
            for (noOfOptions in optionValues.indices) {
                ...
                eachCheckBoxTypeOptionValue.setTextColor(ContextCompat.getColor(this, R.color.text_color_primary))
    ...
    private fun loadCustomOptionDateType(customOptIndex: Int) {
        try {
            ...
            val customOptionDateView = layoutInflater.inflate(R.layout.custom_option_date_view, mContentViewBinding.productCustomOptionsContainer, false)
            ...
            dateEditText1.setOnClickListener { DatePickerDialog(this, R.style.AlertDialogTheme, date1, myCalendar.get(Calendar.YEAR), myCalendar.get(Calendar.MONTH), myCalendar.get(Calendar.DAY_OF_MONTH)).show() }
    ...
    private fun loadCustomOptionTimeType(customOptIndex: Int) {
        try {
            val customOptionTimeView = layoutInflater.inflate(R.layout.custom_option_time_view, mContentViewBinding.productCustomOptionsContainer, false)
            ...
            timeEditText1.setOnClickListener {
                ...
                mTimePicker = TimePickerDialog(this, TimePickerDialog.OnTimeSetListener { _, selectedHour, selectedMinute ->
                    ...
                    timeEditText1.text = String.format(Locale.US, "%02d", currentHour) + ":" + String.format(Locale.US, "%02d", selectedMinute) + " " + aMpM
    ...
    private fun loadCustomOptionDateTimeType(customOptIndex: Int) {
        try {
            val customOptionDateLayout = layoutInflater.inflate(R.layout.custom_option_date_view, mContentViewBinding.productCustomOptionsContainer, false)
            ...
            timeEditText1.setOnClickListener {
                ...
                mTimePicker = TimePickerDialog(this, TimePickerDialog.OnTimeSetListener { _, selectedHour, selectedMinute ->
                    ...
                    timeEditText1.text = currentHour.toString() + ":" + selectedMinute + " " + aMpM

                    if (dateEditText1.text.toString() == "") {
                        DatePickerDialog(this, R.style.AlertDialogTheme, date1, myCalendar.get(Calendar.YEAR), myCalendar.get(Calendar.MONTH), myCalendar.get(Calendar.DAY_OF_MONTH)).show()
        ...
        try {
            if (mContentViewBinding.data!!.isAvailable) {
                if (mContentViewBinding.data!!.bundleOptions.size != 0) {
                    mContentViewBinding.otherProductOptionsContainer.setPadding(resources.getDimension(R.dimen.spacing_normal).toInt(), resources.getDimension(R.dimen.spacing_normal).toInt(), resources.getDimension(R.dimen.spacing_normal).toInt(), resources.getDimension(R.dimen.spacing_normal).toInt() - 15)
                    for (bundleOptIndex in 0 until mContentViewBinding.data!!.bundleOptions.size) {
                        val eachBundleItemNameTV = TextView(this)
                        eachBundleItemNameTV.typeface = TypefaceUtils.load(assets, ApplicationConstants.CALLIGRAPHY_FONT_PATH_SEMI_BOLD)
                        eachBundleItemNameTV.textSize = 14f
                        eachBundleItemNameTV.setTextColor(ContextCompat.getColor(this, R.color.text_color_secondary))
                        if (bundleOptIndex == 0)
                            eachBundleItemNameTV.setPadding(0, 0, 0, 15)
                        else
                            eachBundleItemNameTV.setPadding(0, 30, 0, 15)
                        eachBundleItemNameTV.text = mContentViewBinding.data!!.bundleOptions[bundleOptIndex].title
                        if (mContentViewBinding.data!!.bundleOptions[bundleOptIndex].required == 1) {
                            val requiredSign = SpannableString("*")
                            requiredSign.setSpan(ForegroundColorSpan(Color.RED), 0, requiredSign.length, Spannable.SPAN_EXCLUSIVE_EXCLUSIVE)
                            eachBundleItemNameTV.append(requiredSign)
                        }

                        mContentViewBinding.otherProductOptionsContainer.addView(eachBundleItemNameTV)
    ...
    private fun loadDropDownBundleOption(bundleOptIndex: Int) {
        try {
            val bundleOptionTypeDropDownLayout = layoutInflater.inflate(R.layout.bundle_option_drop_down_layout, mContentViewBinding.otherProductOptionsContainer, false)
            ...
            val incrementQty = bundleOptionTypeDropDownLayout.findViewById<AppCompatTextView>(R.id.increment_qty_iv)
            ...
            val decrementQty = bundleOptionTypeDropDownLayout.findViewById<AppCompatTextView>(R.id.decrement_qty_iv)
            ...
            val dropdownBundleOptionSpinner = bundleOptionTypeDropDownLayout.findViewById<AppCompatSpinner>(R.id.dropdown_bundle_option_spinner)

            var defaultSelected = 0
            for (noOfOptions in 0 until mContentViewBinding.data!!.bundleOptions[bundleOptIndex].optionValues.size) {
                val optionString = mContentViewBinding.data!!.bundleOptions[bundleOptIndex].optionValues[noOfOptions].title
                spinnerOptions.add(optionString)
            ...
            dropdownBundleOptionSpinner.onItemSelectedListener = object : AdapterView.OnItemSelectedListener {
                override fun onItemSelected(spinnerView: AdapterView<*>, selectedItemView: View?, position: Int, id: Long) {
                    try {
                        ...
                        if (position == 0 || bundleOptionValueKey == "") {
                            ...
                            val defaultQty = mContentViewBinding.data!!.bundleOptions[bundleOptionPosition].optionValues[Integer.parseInt(bundleOptionValueKey)].defaultQty.toInt()
                            if (defaultQty != 0) {
                                qtyET.text = defaultQty.toString()
                            } else {
                                qtyET.text = "1"
                            }
                            val userDefinedQty = mContentViewBinding.data!!.bundleOptions[bundleOptionPosition].optionValues[Integer.parseInt(bundleOptionValueKey)].isQtyUserDefined
                            if (userDefinedQty == 0) {
                                incrementQty.isEnabled = false
                                decrementQty.isEnabled = false
                            } else {
                            ...
                            qtyET.isEnabled = mContentViewBinding.data!!.bundleOptions[bundleOptionPosition].optionValues[Integer.parseInt(bundleOptionValueKey)].isQtyUserDefined == 1
                        }
                        updatePrice()
                    } catch (e: Exception) {
                        e.printStackTrace()
    ...
    private fun loadRadioBundleOption(bundleOptIndex: Int) {
        try {
            ...
            val bundleOptionTypeRadioLayout = layoutInflater.inflate(R.layout.bundle_option_radio_layout, mContentViewBinding.otherProductOptionsContainer, false)
            ...
            val bundleOptionRG = bundleOptionTypeRadioLayout.findViewById<RadioGroup>(R.id.bundle_option_rg)
            bundleOptionRG.tag = "bundleoption$bundleOptIndex"

            val incrementQty = bundleOptionTypeRadioLayout.findViewById<AppCompatTextView>(R.id.increment_qty_iv)
            incrementQty.tag = "bundleoption$bundleOptIndex"
            incrementQty.setOnClickListener { v ->
                val editText = mContentViewBinding.otherProductOptionsContainer.findViewWithTag<LinearLayoutCompat>(v.tag).findViewById<AppCompatTextView>(R.id.qty_et)
                var currentQty = 0
                try {
            ...
            val decrementQty = bundleOptionTypeRadioLayout.findViewById<AppCompatTextView>(R.id.decrement_qty_iv)
            ...
            for (noOfOptions in 0 until mContentViewBinding.data!!.bundleOptions[bundleOptIndex].optionValues.size) {
                ...
                if (mContentViewBinding.data!!.bundleOptions[bundleOptIndex].optionValues[noOfOptions].isDefault == "1") {
                    ...
                    qtyET.text = mContentViewBinding.data!!.bundleOptions[bundleOptIndex].optionValues[noOfOptions].defaultQty.toString()
                    incrementQty.isEnabled = mContentViewBinding.data!!.bundleOptions[bundleOptIndex].optionValues[noOfOptions].isQtyUserDefined == 1
                    decrementQty.isEnabled = mContentViewBinding.data!!.bundleOptions[bundleOptIndex].optionValues[noOfOptions].isQtyUserDefined == 1
                }
            }
                ...
                try {
                    if (checkedId != -1) {
                        ...
                        if (bundleOptionValueKey.isEmpty()) {
                            ...
                            val defaultQty = mContentViewBinding.data!!.bundleOptions[bundleOptIndex].optionValues[Integer.parseInt(bundleOptionValueKey)].defaultQty.toInt()
                            if (defaultQty != 0) {
                                qtyET.text = defaultQty.toString()
                            } else {
                                qtyET.text = "1"
                        ...
                        incrementQty.isEnabled = mContentViewBinding.data!!.bundleOptions[bundleOptIndex].optionValues[Integer.parseInt(bundleOptionValueKey)].isQtyUserDefined == 1
                        decrementQty.isEnabled = mContentViewBinding.data!!.bundleOptions[bundleOptIndex].optionValues[Integer.parseInt(bundleOptionValueKey)].isQtyUserDefined == 1
                        updatePrice()
                    }
                } catch (e: Exception) {
                    e.printStackTrace()
    ...
    private fun loadCheckAndMultiBundleOption(bundleOptIndex: Int) {
        try {
            ...
            for (noOfOptions in 0 until mContentViewBinding.data!!.bundleOptions[bundleOptIndex].optionValues.size) {
                ...
                eachCheckTypeBundleOptionValue.isChecked = mContentViewBinding.data!!.bundleOptions[bundleOptIndex].optionValues[noOfOptions].isDefault == "1"
            ...
            mContentViewBinding.otherProductOptionsContainer.addView(bundleOptionTypeCheckMultiLayout)
        ...
        try {
            if (mContentViewBinding.data!!.isAvailable) {
                if (mContentViewBinding.data!!.groupedData.size != 0) {
                    ...
                    mContentViewBinding.otherProductOptionsContainer.setPadding(resources.getDimension(R.dimen.spacing_normal).toInt(), resources.getDimension(R.dimen.spacing_normal).toInt(), resources.getDimension(R.dimen.spacing_normal).toInt(), resources.getDimension(R.dimen.spacing_normal).toInt())
                    for (noOfGroupedData in 0 until mContentViewBinding.data!!.groupedData.size) {
                        val eachGroupProductItem = layoutInflater.inflate(R.layout.item_grouped_product, mContentViewBinding.otherProductOptionsContainer, false)
                        eachGroupProductItem.tag = noOfGroupedData


                        if (!mContentViewBinding.data!!.groupedData[noOfGroupedData].isAvailable) {
                            eachGroupProductItem.findViewById<LinearLayoutCompat>(R.id.quantity_container).visibility = View.GONE
                            val editText = eachGroupProductItem.findViewById<AppCompatTextView>(R.id.qty)
                            editText.text = 0.toString()
                            eachGroupProductItem.findViewById<AppCompatTextView>(R.id.outofstock).visibility = View.VISIBLE
                        }

                        (eachGroupProductItem.findViewById(R.id.nameOfGroupedData) as AppCompatTextView).text = mContentViewBinding.data!!.groupedData[noOfGroupedData].name

                        if (mContentViewBinding.data!!.groupedData[noOfGroupedData].isInRange && !mContentViewBinding.data!!.groupedData[noOfGroupedData].specialPrice.equals("$0.00", true)) {
                            (eachGroupProductItem.findViewById(R.id.priceOfGroupedProduct) as AppCompatTextView).text = mContentViewBinding.data!!.groupedData[noOfGroupedData].specialPrice
                            (eachGroupProductItem.findViewById(R.id.specialpriceOfGroupedProduct) as AppCompatTextView).text = mContentViewBinding.data!!.groupedData[noOfGroupedData].formattedPrice

                            (eachGroupProductItem.findViewById(R.id.specialpriceOfGroupedProduct) as AppCompatTextView).paintFlags = (eachGroupProductItem.findViewById(R.id.priceOfGroupedProduct) as TextView).paintFlags or Paint.STRIKE_THRU_TEXT_FLAG
                            eachGroupProductItem.findViewById<AppCompatTextView>(R.id.specialpriceOfGroupedProduct).visibility = View.VISIBLE
                        } else {
                            (eachGroupProductItem.findViewById(R.id.priceOfGroupedProduct) as TextView).text = mContentViewBinding.data!!.groupedData[noOfGroupedData].formattedPrice
                        }

                        val editText = eachGroupProductItem.findViewById<AppCompatTextView>(R.id.qty)
                        editText.text = mContentViewBinding.data!!.groupedData[noOfGroupedData].defaultQty

                        val incrementQty = eachGroupProductItem.findViewById<AppCompatTextView>(R.id.increment_qty_iv)
                        incrementQty.tag = noOfGroupedData
                        incrementQty.setOnClickListener {
                            val currentQty = Integer.parseInt(editText.text.toString()) + 1
                            editText.text = currentQty.toString()
                        }
                        val decrementQty = eachGroupProductItem.findViewById<AppCompatTextView>(R.id.decrement_qty_iv)
                        ...
                        val eachGroupedProductImage = eachGroupProductItem.findViewById<AppCompatImageView>(R.id.grouped_product_image)
                        ImageHelper.load(eachGroupedProductImage, mContentViewBinding.data!!.groupedData[noOfGroupedData].thumbNail, null)
                        mContentViewBinding.otherProductOptionsContainer.addView(eachGroupProductItem)
                    }
                }
            }
        } catch (e: Exception) {
    ...
    private fun loadDownloadableProductData() {
        try {
            if (mContentViewBinding.data!!.isAvailable) {
                ...
                linksTitle.typeface = TypefaceUtils.load(assets, ApplicationConstants.CALLIGRAPHY_FONT_PATH_SEMI_BOLD)
                ...
                for (linkIndex in 0 until mContentViewBinding.data!!.links.linkData.size) {
                    val eachLinkCheckBox = layoutInflater.inflate(R.layout.item_links_downloadable_product, mContentViewBinding.otherProductOptionsContainer, false) as CheckBox
                    eachLinkCheckBox.text = mContentViewBinding.data!!.links.linkData[linkIndex].linkTitle
                    eachLinkCheckBox.textSize = 14f
                    eachLinkCheckBox.setTextColor(ContextCompat.getColor(this, R.color.text_color_primary))
                    if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP) {
                        eachLinkCheckBox.buttonTintList = ColorStateList.valueOf(ContextCompat.getColor(this, R.color.text_color_primary))
                    }
                    ...
                    if (mContentViewBinding.data!!.links.linksPurchasedSeparately == 0) {
                        ...
                        eachLinkCheckBox.text = "${eachLinkCheckBox.text} (+${mContentViewBinding.data!!.links.linkData[linkIndex].formattedPrice})"
                    }
            ...
            if (mContentViewBinding.data!!.samples.hasSample) {
                ...
                sampleTitle.typeface = TypefaceUtils.load(assets, ApplicationConstants.CALLIGRAPHY_FONT_PATH_SEMI_BOLD)
                ...
                for (linkDataIndex in 0 until mContentViewBinding.data!!.samples.linkSampleData.size) {
                    ...
                    sampleLink.setPadding(resources.getDimension(R.dimen.spacing_generic).toInt() + 15, 15, resources.getDimension(R.dimen.spacing_generic).toInt() + 15, 0)
                    sampleLink.textSize = 16f
                    sampleLink.text = mContentViewBinding.data!!.samples.linkSampleData[linkDataIndex].sampleTitle
                    ...
                    sampleLink.setOnClickListener { v ->
                        if (checkPermissions()) {
                            val sampleLinkData = mContentViewBinding.data!!.samples.linkSampleData[v.tag as Int]
                            downloadFile(this, sampleLinkData.url, sampleLinkData.fileName, sampleLinkData.mimeType)
                        }
                    }
    ...
    private fun checkPermissions(): Boolean {
        if (ActivityCompat.checkSelfPermission(this, Manifest.permission.WRITE_EXTERNAL_STORAGE) != PackageManager.PERMISSION_GRANTED) {
            if (ActivityCompat.shouldShowRequestPermissionRationale(this, Manifest.permission.WRITE_EXTERNAL_STORAGE)) {
                ...
                ActivityCompat.requestPermissions(this, arrayOf(Manifest.permission.WRITE_EXTERNAL_STORAGE), RC_WRITE_TO_EXTERNAL_STORAGE)
            } else {
                ActivityCompat.requestPermissions(this, arrayOf(Manifest.permission.WRITE_EXTERNAL_STORAGE), RC_WRITE_TO_EXTERNAL_STORAGE)
            }
            return false
    ...
    private fun addVisualTypeSwatch(index: Int, element: Attribute) {
        val eachConfigurableAttributeLayout = layoutInflater.inflate(R.layout.item_configurable_product_attribute_swatch_layout, mContentViewBinding.otherProductOptionsContainer, false)
        eachConfigurableAttributeLayout.tag = "config$index"

        (eachConfigurableAttributeLayout.findViewById(R.id.configurable_attribute_label) as TextView).typeface = TypefaceUtils.load(assets, ApplicationConstants.CALLIGRAPHY_FONT_PATH_SEMI_BOLD)
        (eachConfigurableAttributeLayout.findViewById(R.id.configurable_attribute_label) as TextView).text = "${element.label}*"
        (eachConfigurableAttributeLayout.findViewById(R.id.configurable_attribute_label) as TextView).isAllCaps = true

        val eachSwatchRecyclerView = eachConfigurableAttributeLayout.findViewById(R.id.configurable_item_rv) as RecyclerView
        ...
        eachSwatchRecyclerView.addItemDecoration(HorizontalMarginItemDecoration(resources.getDimension(R.dimen.spacing_generic).toInt()))
    ...
    private fun addDropDownTypeOptions(index: Int, element: Attribute) {
        val eachConfigurableAttributeLayout = layoutInflater.inflate(R.layout.item_configurable_product_attribute_dropdown_layout, mContentViewBinding.otherProductOptionsContainer, false)
        eachConfigurableAttributeLayout.tag = "config$index"
        ...
        val eachConfigurableAttributeSpinner = eachConfigurableAttributeLayout.findViewById<AppCompatSpinner>(R.id.spinner_configurable_item)
        ...
        eachConfigurableAttributeSpinner.adapter = SpinnerWithTagAdapter(spinnerOptions, spinnerTags)
        mContentViewBinding.otherProductOptionsContainer.addView(eachConfigurableAttributeLayout)
    }
    ...
    fun initializeConfigurableAttributeOption(position: Int) {
        try {
            if (mContentViewBinding.data!!.configurableData.attributes?.get(position)?.swatchType == "visual" || mContentViewBinding.data!!.configurableData.attributes?.get(position)?.swatchType == "text") {
                val eachSwatchRecyclerView = mContentViewBinding.otherProductOptionsContainer.findViewWithTag<LinearLayoutCompat>("config$position").findViewById<RecyclerView>(R.id.configurable_item_rv)
                ...
                val currentConfigurableAttributeSpinner = mContentViewBinding.otherProductOptionsContainer.findViewWithTag<LinearLayoutCompat>("config$position").findViewById<AppCompatSpinner>(R.id.spinner_configurable_item)
                val spinnerOptions = ArrayList<String>()
                val spinnerTags = ArrayList<String>()
                mContentViewBinding.data!!.configurableData.chooseText?.let { spinnerOptions.add(it) }
                spinnerTags.add("")

                for (attributeOptionPosition in 0 until mContentViewBinding.data!!.configurableData.attributes?.get(position)?.options?.size!!) {

                    if (canAddCurrentAttributeOption(position, attributeOptionPosition)) {
                        mContentViewBinding.data!!.configurableData.attributes?.get(position)?.options?.get(attributeOptionPosition)?.label?.let { spinnerOptions.add(it) }
                        spinnerTags.add(attributeOptionPosition.toString())
                    }
                }
                currentConfigurableAttributeSpinner.adapter = SpinnerWithTagAdapter(spinnerOptions, spinnerTags)
                currentConfigurableAttributeSpinner.onItemSelectedListener = object : AdapterView.OnItemSelectedListener {
                    override fun onItemSelected(parent: AdapterView<*>, selectedItemView: View?, selectedItemPosition: Int, id: Long) {
                        try {
                            parent.let {
    ...
    private fun canAddCurrentAttributeOption(currentAttributePositionToUpdate: Int, attributeOptionPosition: Int): Boolean {
        try {
            ...
            if (currentAttributePositionToUpdate == 0) {
                return mContentViewBinding.data!!.configurableData.attributes?.get(currentAttributePositionToUpdate)?.options?.get(attributeOptionPosition)?.products?.size != 0
            }

            val currentProductList = mContentViewBinding.data!!.configurableData.attributes?.get(currentAttributePositionToUpdate)?.options?.get(attributeOptionPosition)?.products
            ...
            for (noOfProductAssociatedWithCurrentAttributeOption in currentProductList?.indices!!) {
                ...
                for (noOfAttributeBeforeTheCurrentAttribute in 0 until currentAttributePositionToUpdate) {
                    if (mContentViewBinding.data!!.configurableData.attributes?.get(noOfAttributeBeforeTheCurrentAttribute)?.swatchType == "visual" || mContentViewBinding.data!!.configurableData.attributes?.get(noOfAttributeBeforeTheCurrentAttribute)?.swatchType == "text") {
                        val eachSwatchRecyclerView = mContentViewBinding.otherProductOptionsContainer.findViewWithTag<LinearLayoutCompat>("config$noOfAttributeBeforeTheCurrentAttribute").findViewById<RecyclerView>(R.id.configurable_item_rv)
                        val eachRecyclerViewData = (eachSwatchRecyclerView.adapter as ProductAttributesSwatchRvAdapter).getSwatchValuesData()
                        ...
                        for (noOfData in eachRecyclerViewData.indices) {
                            if (eachRecyclerViewData[noOfData].isSelected) {
                                actualPositionOfEachConfigurableProductAttributeSpinnerBeforeCurrentAttribute = noOfData.toString()
                            }
                        }
                        if (actualPositionOfEachConfigurableProductAttributeSpinnerBeforeCurrentAttribute.isEmpty()) {
                            return mContentViewBinding.data!!.configurableData.attributes?.get(currentAttributePositionToUpdate)?.swatchType == "visual" || this.mContentViewBinding.data!!.configurableData.attributes?.get(currentAttributePositionToUpdate)?.swatchType == "text"
                        }
                        val eachProductArrayOfAttributeBeforeTheCurrentAttribute = mContentViewBinding.data!!.configurableData.attributes?.get(noOfAttributeBeforeTheCurrentAttribute)?.options?.get(Integer.parseInt(actualPositionOfEachConfigurableProductAttributeSpinnerBeforeCurrentAttribute))?.products
                        for (i in eachProductArrayOfAttributeBeforeTheCurrentAttribute?.indices!!) {
                            if (eachProductArrayOfAttributeBeforeTheCurrentAttribute.get(i) == currentProductList.get(noOfProductAssociatedWithCurrentAttributeOption)) {
                                noOfMatches++
                            }
                        }
                    } else {
                        val eachConfigurableProductAttributeSpinnerBeforeCurrentAttribute = mContentViewBinding.otherProductOptionsContainer.findViewWithTag<LinearLayoutCompat>("config$noOfAttributeBeforeTheCurrentAttribute").findViewById<AppCompatSpinner>(R.id.spinner_configurable_item)
                        val actualPositionOfEachConfigurableProductAttributeSpinnerBeforeCurrentAttribute = eachConfigurableProductAttributeSpinnerBeforeCurrentAttribute.selectedItem as String
                        if (actualPositionOfEachConfigurableProductAttributeSpinnerBeforeCurrentAttribute.isEmpty()) {
                            return mContentViewBinding.data!!.configurableData.attributes?.get(currentAttributePositionToUpdate)?.swatchType == "visual" || mContentViewBinding.data!!.configurableData.attributes?.get(currentAttributePositionToUpdate)?.swatchType == "text"
                        }
                        val eachProductArrayOfAttributeBeforeTheCurrentAttribute = mContentViewBinding.data!!.configurableData.attributes?.get(noOfAttributeBeforeTheCurrentAttribute)?.options?.get(Integer.parseInt(actualPositionOfEachConfigurableProductAttributeSpinnerBeforeCurrentAttribute))?.products
                        for (i in eachProductArrayOfAttributeBeforeTheCurrentAttribute?.indices!!) {
                            if (eachProductArrayOfAttributeBeforeTheCurrentAttribute.get(i) == currentProductList.get(index = noOfProductAssociatedWithCurrentAttributeOption)) {
                                noOfMatches++
                            }
                        }
                    }
        ...
        try {
            for (attributeOptionPosition in position until (mContentViewBinding.data!!.configurableData.attributes?.size
                ...
                if (mContentViewBinding.data!!.configurableData.attributes?.get(position)?.swatchType == "visual" || mContentViewBinding.data!!.configurableData.attributes?.get(position)?.swatchType == "text") {
                    ...
                    val eachConfigurableAttributeSpinner = mContentViewBinding.otherProductOptionsContainer.findViewWithTag<LinearLayoutCompat>(attributeOptionPosition).findViewById<AppCompatSpinner>(R.id.spinner_configurable_item)
                    val spinnerOpions = ArrayList<String>()
                    mContentViewBinding.data!!.configurableData.chooseText?.let { spinnerOpions.add(it) }
                    val spinnerArrayAdapter = ArrayAdapter(this, android.R.layout.simple_spinner_dropdown_item, spinnerOpions)
    ...
    fun updatePrice() = try {
        ...
        if (mContentViewBinding.data!!.configurableData.attributes?.size ?: 0 > 0) {
            ...
            for (noOfConfigData in 0 until (mContentViewBinding.data!!.configurableData.attributes?.size
                    ?: 0)) {
                mContentViewBinding.data!!.configurableData.attributes?.get(noOfConfigData)?.id?.let { configOptionsId.add(it) }
                if (mContentViewBinding.data!!.configurableData.attributes?.get(noOfConfigData)?.swatchType == "visual" || mContentViewBinding.data!!.configurableData.attributes?.get(noOfConfigData)?.swatchType == "text") {
                    val eachSwatchRecyclerView = mContentViewBinding.otherProductOptionsContainer.findViewWithTag<LinearLayoutCompat>("config$noOfConfigData").findViewById<RecyclerView>(R.id.configurable_item_rv)
                    val recyclerViewOptions = (eachSwatchRecyclerView.adapter as ProductAttributesSwatchRvAdapter).getSwatchValuesData()
                    ...
                    for (noOfAttribute in recyclerViewOptions.indices) {
                        if (recyclerViewOptions[noOfAttribute].isSelected) {
                            ...
                            noOfSelectedOptions++
                            selectedConfigOptionsId.add(noOfConfigData, recyclerViewOptions[noOfAttribute].id)
                        }
                    }
                    if (!isOptionSelected) {
                    ...
                    val eachConfigurableAttributeSpinner = mContentViewBinding.otherProductOptionsContainer.findViewWithTag<LinearLayoutCompat>("config$noOfConfigData").findViewById<AppCompatSpinner>(R.id.spinner_configurable_item)
                    val selectedItemOfEachConfigurableProductAttributeSpinner = eachConfigurableAttributeSpinner.selectedItemPosition
                    val actualPositionOfEachConfigurableProductAttributeSpinner = eachConfigurableAttributeSpinner.getItemAtPosition(selectedItemOfEachConfigurableProductAttributeSpinner) as String

                    if (actualPositionOfEachConfigurableProductAttributeSpinner.isEmpty()) {
                        mContentViewBinding.data!!.configurableData.attributes?.get(noOfConfigData)?.options?.get(0)?.id?.let { selectedConfigOptionsId.add(noOfConfigData, it) }
                    } else {
                        noOfSelectedOptions++
                        mContentViewBinding.data!!.configurableData.attributes?.get(noOfConfigData)?.options?.get(Integer.parseInt(actualPositionOfEachConfigurableProductAttributeSpinner))?.id?.let { selectedConfigOptionsId.add(noOfConfigData, it) }
                    }
                }
            }
            ...
            for (noOfCombinationData in 0 until indexArray.length()) {
                for (noOfIndexItems in configOptionsId.indices) {
                    if (indexArray.getJSONObject(noOfCombinationData).getString(configOptionsId[noOfIndexItems]) == selectedConfigOptionsId[noOfIndexItems]) {
                        priceFound = true
                    } else {
                        priceFound = false
                        break
                ...
                if (priceFound) {
                    updatedFinalPrice = mContentViewBinding.data!!.configurableData.optionPrices?.get(noOfCombinationData)?.finalPrice?.amount
                            ?: 0.0
                    productId = mContentViewBinding.data!!.configurableData.optionPrices?.get(noOfCombinationData)?.product
                            ?: 0
                    break
                }
            }
        ...
        if (mContentViewBinding.data!!.customOptions.size > 0) {
            for (noOfCustomOpt in 0 until mContentViewBinding.data!!.customOptions.size) {
                when (mContentViewBinding.data!!.customOptions[noOfCustomOpt].type) {
                    "field", "area" -> {
                        val customOptionFieldView = mContentViewBinding.productCustomOptionsContainer.findViewWithTag<AppCompatEditText>(noOfCustomOpt)
                        if (!(customOptionFieldView as AppCompatEditText).text.toString().matches("".toRegex())) {
                    ...
                    "file" -> if ((mContentViewBinding.productCustomOptionsContainer.findViewWithTag<LinearLayoutCompat>(noOfCustomOpt).findViewById<AppCompatTextView>(R.id.fileSelectedTV) as TextView).text != resources.getString(R.string.no_file_selected)) {
                        updatedFinalPrice += (if (mContentViewBinding.data!!.customOptions[noOfCustomOpt].price_type == "fixed") {
                            mContentViewBinding.data!!.customOptions[noOfCustomOpt].default_price
                        } else {
                            mContentViewBinding.data!!.customOptions[noOfCustomOpt].default_price!! * finalPrice / 100
                    ...
                    "drop_down" -> {
                        val spinnerItem = mContentViewBinding.productCustomOptionsContainer.findViewWithTag<AppCompatSpinner>(noOfCustomOpt)
                        val positionOfSpinnerItem = spinnerItem.selectedItemPosition
                        if (positionOfSpinnerItem != 0) {
                            updatedFinalPrice += if (mContentViewBinding.data!!.customOptions[noOfCustomOpt].optionValues!!.get(positionOfSpinnerItem - 1).priceType == "fixed") {
                                mContentViewBinding.data!!.customOptions[noOfCustomOpt].optionValues?.get(positionOfSpinnerItem - 1)!!.defaultPrice
                            } else {
                    ...
                    "radio" -> {
                        val radioGroup = mContentViewBinding.productCustomOptionsContainer.findViewWithTag<RadioGroup>(noOfCustomOpt)
                        if (radioGroup.checkedRadioButtonId != -1) {
                            val selectedRadioBtn = mContentViewBinding.root.findViewById<RadioButton>(radioGroup.checkedRadioButtonId)
                    ...
                    "checkbox", "multiple" -> {
                        val checkBoxContainerLL = mContentViewBinding.productCustomOptionsContainer.findViewWithTag<LinearLayoutCompat>(noOfCustomOpt)
                    ...
                    "date" -> {
                        val dateET = mContentViewBinding.productCustomOptionsContainer.findViewWithTag<LinearLayoutCompat>(noOfCustomOpt).findViewById<AppCompatTextView>(R.id.dateET)
                    ...
                    "time" -> {
                        val timeET = mContentViewBinding.productCustomOptionsContainer.findViewWithTag<LinearLayoutCompat>(noOfCustomOpt).findViewById<AppCompatTextView>(R.id.timeET)
                    ...
                    "date_time" -> {
                        val v = mContentViewBinding.productCustomOptionsContainer.findViewWithTag<LinearLayoutCompat>("date$noOfCustomOpt").findViewById<AppCompatTextView>(R.id.dateET)
                        if (!v.text.toString().matches("".toRegex())) {
                            updatedFinalPrice += mContentViewBinding.data!!.customOptions[noOfCustomOpt].default_price!!
                        }
                        if (v.text.toString().matches("".toRegex())) {
                            val asscociatedET = mContentViewBinding.productCustomOptionsContainer.findViewWithTag<LinearLayoutCompat>("time$noOfCustomOpt").findViewById<AppCompatTextView>(R.id.timeET)
...
/*
    ...
    private fun updateImages(productId: Int) {
        ...
        for (noOfImages in 0 until mContentViewBinding.data!!.imageGallery.size) {
            ...
            imageGalleryDataObject.smallImage = mContentViewBinding.data!!.imageGallery[noOfImages].smallImage
            imageGalleryDataObject.largeImage = mContentViewBinding.data!!.imageGallery[noOfImages].largeImage
            ...
            try {
                ...
                if (imageGalleryArray != null) {
                    for (noOfImages in 0 until imageGalleryArray.length()) {
                        val imageGalleryDataObject = ImageGalleryData()
                        imageGalleryDataObject.smallImage = imageGalleryArray.getJSONObject(noOfImages).getString("thumb")
                        imageGalleryDataObject.largeImage = imageGalleryArray.getJSONObject(noOfImages).getString("full")
        ...
        mContentViewBinding.productSliderViewPager.adapter = ProductPageSliderAdapter(this, mContentViewBinding.data!!.name, imageGalleryDataArray)
        mContentViewBinding.productSliderDotsTabLayout.setupWithViewPager(mContentViewBinding.productSliderViewPager, true)
    }
    ...
    private fun setupReviewsLayoutAndRv() {
        ...
        if (mContentViewBinding.reviewsRv.adapter == null) {
            mContentViewBinding.reviewsRv.addItemDecoration(DividerItemDecoration(this, LinearLayout.VERTICAL))
        }
        mContentViewBinding.reviewsRv.adapter = ReviewsRvAdapter(this, mContentViewBinding.data!!.reviewList)
    }
    ...
    private fun setupRelatedProductsRv() {
        if (mContentViewBinding.relatedProductsRv.adapter == null) {
            mContentViewBinding.relatedProductsRv.addItemDecoration(HorizontalMarginItemDecoration(resources.getDimension(R.dimen.spacing_tiny).toInt()))
            mContentViewBinding.relatedProductsRv.isNestedScrollingEnabled = false
        }
        mContentViewBinding.relatedProductsRv.adapter = RelatedProductsRvAdapter(this, mContentViewBinding.data!!.relatedProductList)
    }

    private fun setupUpsellProductsRv() {
        if (mContentViewBinding.upsellProductsRv.adapter == null) {
            mContentViewBinding.upsellProductsRv.addItemDecoration(HorizontalMarginItemDecoration(resources.getDimension(R.dimen.spacing_tiny).toInt()))
            mContentViewBinding.upsellProductsRv.isNestedScrollingEnabled = false
        }
        mContentViewBinding.upsellProductsRv.adapter = ProductCarouselHorizontalRvAdapter(this, mContentViewBinding.data!!.upsellProductList)
    }
    ...
    private fun setupFloatingBuyLayoutHiding() {
        ...
        mContentViewBinding.scrollView.setOnScrollChangeListener(NestedScrollView.OnScrollChangeListener { _, _, scrollY, _, _ ->
            if (!mContentViewBinding.data!!.isAvailable || (mContentViewBinding.staticBuyLayout.getGlobalVisibleRect(rect) && mContentViewBinding.staticBuyLayout.height == rect.height() &&
        ...
        if (resultCode == Activity.RESULT_OK && data != null) {
            if (requestCode == RC_PICK_SINGLE_FILE) {
                ...
                try {
                    ...
                    (mContentViewBinding.productCustomOptionsContainer.findViewWithTag<LinearLayoutCompat>(mSeletedCustomOption).findViewById(R.id.fileSelectedTV) as TextView).text = fileUri.lastPathSegment
    ...
    inner class SpinnerWithTagAdapter internal constructor(private val mSpinnerOptions: ArrayList<String>, private val spinnerOptionsKeys: ArrayList<String>) : BaseAdapter() {
        ...
        override fun getView(position: Int, convertView: View?, parent: ViewGroup?): View {
            val spinnerItem = layoutInflater.inflate(android.R.layout.simple_spinner_dropdown_item, null) as CheckedTextView
            spinnerItem.setPadding(5, 5, 5, 5)
            spinnerItem.minHeight = TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_DIP, 50f, resources.displayMetrics).toInt()
