...
class OrderShipmentDetailsBottomSheetFragment : FullScreenBottomSheetDialogFragment() {
    ...
    private fun startInitialization() {
        ...
        shippmentId=arguments!!.getString(BUNDLE_KEY__ID_FROM_ORDER_DETAIL,"")
        callApi()
    ...
    private fun callApi() {
        ...
        (context as BaseActivity).mHashIdentifier = Utils.getMd5String("getShipmentDetailsData" + AppSharedPref.getStoreId(context!!) + AppSharedPref.getCustomerToken(context!!) + mContentViewBinding.shipmentId!!)
        ApiConnection.getShipmentDetailsData(context!!, BaseActivity.mDataBaseHandler.getETagFromDatabase((context as BaseActivity).mHashIdentifier), shippmentId)
                .observeOn(AndroidSchedulers.mainThread())
                .subscribeOn(Schedulers.io())
                .subscribe(object : ApiCustomCallback<ShipmentDetailsData>(context!!, true) {
